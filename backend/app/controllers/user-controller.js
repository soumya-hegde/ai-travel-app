...
const { userUpdateValidationSchema, changePasswordValidationSchema, resetPasswordValidationSchema } = require('../validators/validation');
...
userCtlr.userUpdate = async (req, res) =>{
    const body = req.body;
    const {error, value} = userUpdateValidationSchema.validate(body, {abortEarly:false});
    if(error){
    ...
    try{
        ...
        const updatedUser = await User.findByIdAndUpdate(userId, { $set:userUpdatedFields }, { new:true ,runValidators: true} ).select("-password"); // Exclude password, runValidators: true -if the duplicate emails are there it should goto catch block
        res.status(201).json({success: true,
        message: "User Details Updated Successfully"});

    }catch(error){
        if(error.code === 11000){
            return res.status(200).json({success:true, message:"Email already taken, please login!"});
        }
        console.log(error);
...
userCtlr.updateUserPassword = async (req, res) => {
  try {
    ...
    const {error, value} = changePasswordValidationSchema.validate(body, {abortEarly:false});
    if(error){
        return res.status(400).json({error:error.details.map(err=>err.message)});
    }
...
userCtlr.resetPassword = async (req, res) => {
  try {
    const body = req.body;
    const { error, value } = resetPasswordValidationSchema.validate(body, {abortEarly:false});
    if(error){
        return res.status(400).json({error:error.details.map(err=>err.message)});
    }
