...
const {userRegistrationValidationSchema, userLoginValidationSchema} = require('../validators/validation');
const authCtlr = {};

// admin, user register
authCtlr.register = async (req, res) =>{
    const body = req.body;
    const {error, value} = userRegistrationValidationSchema.validate(body, {abortEarly:false});
    if(error){
...
authCtlr.login = async (req, res) => {
    const body = req.body;
    const {value, error} = userLoginValidationSchema.validate(body, {abortEarly:false});
    if(error){
