...
agentCtlr.register = async (req, res) => {
  const body = req.body;
  const { error, value } = await agentRegisterValidationSchema.validate(body, {abortEarly:false});
  if(error){
    return res.status(500).json({error:error.details.map(err => err.message)});
  }
...
agentCtlr.agentUpdate = async (req, res) =>{
    ...
    try{
        ...
        const updatedUser = await User.findByIdAndUpdate(agentId, { $set:userUpdatedFields }, { new:true ,runValidators: true} ).select("-password"); // Exclude password, runValidators: true -if the duplicate emails are there it should goto catch block
